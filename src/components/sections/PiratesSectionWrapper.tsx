...
import PiratesSectionClient from "./PiratesSectionClient";
import { seriesCache } from "@/lib/seriesCache";

const SERIES_ID = "689b0872ac64d856c5d1b111";

export default function PiratesSectionWrapper() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Error fetching Pirates data:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return <PiratesSectionClient seriesName={series.name} movies={movies} />;
}
